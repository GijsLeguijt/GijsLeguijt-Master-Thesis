...
G4cout << "Doet het nog" << G4endl;
  while(counter == 0 || std::abs(buffer-result/std::max(1,counter)) > 0.03*buffer)
  {
    ...
    while (counter<size)
    {
      ...
      G4double theEkin = boosted.GetKineticEnergy();G4cout << "Nog steeds" << G4endl;
      aXsection = (*((*theCrossSections)(index))).GetValue(theEkin, outOfRange);G4cout << "1" << G4endl;
      // velocity correction.
      G4ThreeVector targetVelocity = 1./aThermalNuc.GetMass()*aThermalNuc.GetMomentum();G4cout << "2" << G4endl;
      aXsection *= (targetVelocity-neutronVelocity).mag()/neutronVMag;G4cout << "3" << G4endl;
      result += aXsection;
    }
    size += size;
  }
